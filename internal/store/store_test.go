...
func TestAddAndGetEdges(t *testing.T) {
	...
	out := s.GetOutboundEdges("/a")
	if len(out) != 1 || out[0].To != "/b" {
		t.Fatalf("expected 1 outbound edge to /b, got %v", out)
	}

	in := s.GetInboundEdges("/b")
	if len(in) != 1 || in[0].From != "/a" {
		t.Fatalf("expected 1 inbound edge from /a, got %v", in)
	}
}
...
func TestHasEdge(t *testing.T) {
	s := store.NewMemoryStore()
	if s.HasEdge("/a", "/b") {
		t.Fatal("should not have edge yet")
	}
	s.AddEdge(model.Edge{From: "/a", To: "/b", Type: model.EdgeDependsOn})
	if !s.HasEdge("/a", "/b") {
		t.Fatal("should have edge after AddEdge")
	}
}
...
func TestRemoveEdge(t *testing.T) {
	...
	if len(s.GetOutboundEdges("/a")) != 0 {
		t.Fatal("outbound edges should be empty")
	}
	if len(s.GetInboundEdges("/b")) != 0 {
		t.Fatal("inbound edges should be empty")
	}
}

func TestDeleteNode_CleansEdges(t *testing.T) {
	s := store.NewMemoryStore()
	s.PutNode(&model.Node{Header: model.Header{Path: "/a", Status: model.StatusActive, Fields: map[string]string{}}})
	...
	if len(s.GetOutboundEdges("/a")) != 0 {
		t.Fatal("outbound edges of deleted node should be gone")
	}
	if len(s.GetInboundEdges("/b")) != 0 {
		t.Fatal("inbound edges referencing deleted node should be gone")
	}
}
...
func TestMultipleEdges(t *testing.T) {
	s := store.NewMemoryStore()
	s.AddEdge(model.Edge{From: "/a", To: "/b", Type: model.EdgeDependsOn})
	s.AddEdge(model.Edge{From: "/a", To: "/c", Type: model.EdgeReferences})
